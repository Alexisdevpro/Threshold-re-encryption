...
use crate::lwe_functions::{generate_alea_encrypt, keygen, PublicKey};
...
fn encrypt_plaintext_from_file() {
    ...
    let text = plaintext_content.trim().parse::<i64>().unwrap();

    let ciphertext = lwe_functions::encrypt(&public_key, text);
