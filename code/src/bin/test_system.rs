...
fn generate_system_keys_and_shares() {
    ...
    append_vectors_to_files(
        vec![
            ("test_key/share_machine1.txt", vec![&shares_machine_1.data.as_vec()]),
            ("test_key/r_share_machine1.txt", vec![&r_for_machine_1.data.as_vec()]),
        ]
    );
    append_vectors_to_files(
        vec![
            ("test_key/share_machine2.txt", vec![&shares_machine_2.data.as_vec()]),
            ("test_key/r_share_machine2.txt", vec![&r_for_machine_2.data.as_vec()]),
        ]
    );
    append_vectors_to_files(
        vec![
...
fn encrypt_plaintext_from_file() {
    ...
    plaintext_file.read_to_string(&mut plaintext_content).unwrap();
